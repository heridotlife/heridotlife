        ...
        <ul className="w-full space-y-4">
          {categories.length > 0 ? (
            categories.map((category) => (
              <li key={category.id} className="p-4 bg-gray-200 rounded-md shadow-md">
                <Link href={`/category/${category.name}`} className="text-lg font-semibold text-blue-500 underline">
                  {category.name}
                </Link>
              </li>
            ))
