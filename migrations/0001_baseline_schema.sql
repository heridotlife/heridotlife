-- Migration 0001: baseline schema (snapshot of schema.sql at adoption of wrangler d1 migrations).
-- Fully idempotent (IF NOT EXISTS everywhere), so applying it against the existing
-- production database is a no-op that just records the baseline in d1_migrations.
-- For future schema changes: create a new migration with
--   wrangler d1 migrations create D1_db <name>
-- and mirror the change in schema.sql (the canonical snapshot used by tests/local setup).
